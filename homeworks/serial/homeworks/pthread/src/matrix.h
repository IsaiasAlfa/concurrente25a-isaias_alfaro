...
/**
 * @brief Estructura que almacena los datos para la ejecucion de los hilos.
 */
struct hilo_data {
    struct data_job *plate_data;
    struct heat **heat_dat;
    int hilo_id;
    int total_hilos;
    double burn;
};
 ...
 * @param hilos Cantidad de hilos para el proceso.
 */
 ...
 * @param job Nombre del trabajo que se utilizará para generar el archivo de salida.
 * @param hilos Cantidad de hilos que se van a crear.
 */
...
/**
 * @brief Metodo para el uso de los hilos en la matriz.
 * 
 * @param arg puntero con la informacion necesaria para la ejecucion.
 */
